...
def test_control_surfaces_only_allowlisted_regime_quote_reconstruction_family(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    marker_directory = tmp_path / "markers"
    ...
    def fail(*_args, **_kwargs):
        raise MintStateAcceptanceError(
            "secret regime quote detail",
            code="CYCLE_RECONSTRUCTION_COMPONENT_REGIME_QUOTE_FAILED",
        )
    ...
    assert "secret regime quote" not in encoded
...
def test_control_surfaces_only_allowlisted_regime_stage_reconstruction_family(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    analysis_code: str,
) -> None:
    ...
    encoded = json.dumps(result)
    assert result["status"] == "FAILED"
    assert result["error"]["code"] == f"ANALYSIS_{analysis_code}"
    assert result["error"]["message"] == (
        "mint-state acceptance analysis failed closed"
    )
    assert "secret regime stage" not in encoded


def test_control_surfaces_only_allowlisted_regime_reconstruction_family(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    marker_directory = tmp_path / "markers"
    ...
    def fail(*_args, **_kwargs):
        raise MintStateAcceptanceError(
            "secret regime detail",
            code="CYCLE_RECONSTRUCTION_COMPONENT_REGIME_WINDOW_FAILED",
        )
    ...
    assert (
        result["error"]["code"]
        == "ANALYSIS_CYCLE_RECONSTRUCTION_COMPONENT_REGIME_WINDOW_FAILED"
    )
    assert result["error"]["message"] == (
        "mint-state acceptance analysis failed closed"
    )
    assert "secret regime" not in encoded
