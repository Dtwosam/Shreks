    ...
    connection
        .execute_batch(
            "CREATE TABLE token_candidates (id INTEGER, mint TEXT);\n\
             CREATE TABLE market_snapshots (candidate_id INTEGER, observed_at_unix_ms INTEGER);\n\
             INSERT INTO token_candidates (id, mint) VALUES (-1, '   ');\n\
             INSERT INTO market_snapshots (candidate_id, observed_at_unix_ms) VALUES (-1, 900);",
        )
        .unwrap();
    drop(connection);
