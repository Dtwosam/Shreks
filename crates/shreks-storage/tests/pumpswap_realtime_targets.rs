...
#[test]
fn verified_pumpswap_targets_are_point_in_time_bounded_deduplicated_and_deterministic() {
    ...
    let targets = db
        .verified_pumpswap_realtime_targets(1_000, 200, 3)
        .unwrap();
...
#[test]
fn verified_pumpswap_targets_reject_unbounded_or_invalid_queries() {
    let root = unique_test_dir("invalid");
    let db = ShreksDb::open(root.join("shreks.db")).unwrap();

    assert!(db.verified_pumpswap_realtime_targets(-1, 1_000, 1).is_err());
    assert!(db.verified_pumpswap_realtime_targets(1_000, 0, 1).is_err());
    assert!(db.verified_pumpswap_realtime_targets(1_000, 1_000, 0).is_err());
