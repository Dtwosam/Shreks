...
fn holder_distribution_persists_full_width_values_incomplete_state_and_restart_identity() {
    ...
    db.insert_holder_distribution(candidate_id, &complete)
        .unwrap();
    db.insert_holder_distribution(candidate_id, &complete)
        .unwrap();
    ...
    db.insert_holder_distribution(candidate_id, &incomplete)
        .unwrap();
    drop(db);
...
fn holder_distribution_rejects_candidate_mismatch_invalid_invariants_and_conflicting_replay() {
    ...
    assert!(db
        .insert_holder_distribution(candidate_id, &mismatch)
        .is_err());
...
fn exit_quote_rejects_invalid_probe_candidate_or_request_response_mismatch_and_conflicting_replay()
{
    ...
    let connection = Connection::open(&db_path).unwrap();
    let count: i64 = connection
        .query_row("SELECT COUNT(*) FROM exit_quote_snapshots", [], |row| {
            row.get(0)
        })
        .unwrap();
    assert_eq!(count, 1);
